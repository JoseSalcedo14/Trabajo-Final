...
def validar_asis(fecha_str, formato="%d-%m-%y"):
    try:
        fecha = datetime.strptime(fecha_str, formato)
        fecha_actual = datetime.now()
        f_min = fecha_actual - timedelta(days=14)
...
def rutas_mu():
    print("\nRutas ordenadas por cantidad de alumnos")
    rutas = pd.read_csv("datos/rutas.csv", sep=";")

    ordenado = rutas.sort_values('cant_alumnos', ascending=False)
    print(ordenado)
...
def ver_reportes():
    ...
    match opc:
        case 1:
            if not archivos['estudiantes']:
                print("No hay estudiantes registrados en las rutas.\n")
                return
            rutas_mu()
        case 2:
            pass
