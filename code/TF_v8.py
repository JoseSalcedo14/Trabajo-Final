...
def validar_arch():
    ...
    # Rutas
    try:
        rutas = pd.read_csv("datos/rutas.csv")
        if not rutas.empty:
            datos['rutas'] = True
        else:
            datos['rutas'] = False
    ...
    # Asistencias
    try:
        asistencias = pd.read_csv("datos/asistencias.csv")
        if not asistencias.empty:
            datos['asistencias'] = True
        else:
            datos['asistencias'] = False
    except FileNotFoundError:
        asistencias = pd.DataFrame(columns=["dni", "nombres", "apellidos", "ruta"])
...
def control_asis():
    while True:
        try:
            # Se muestra el menú de filtros disponibles.
            print("\n[1] Ver asistencias por fecha")
            print("[2] Ver asistencias por estudiante")
            print("[3] Ver asistencias por ruta")
            print("[4] Volver al menú principal")
            opcion = int(input("Ingrese una opción > "))
            # Se valida que la opción esté entre 1 y 3.
...
def opciones(opc):
    ...
    match opc:
        ...
        case 5:
            if (
                not archivos['conductores']
                or not archivos['estudiantes']
            ):
