...
def r_vehiculo():
    ...
    n_vehiculo.to_csv(
        "datos/vehiculos.csv", sep=";", index=False, mode="a", header=False
    )
...
def r_conductor():
    ...
    try:
        ...
        print("Error: no existe el archivo 'rutas.csv'.\n")
        return
    ...
    try:
        ...
        print("Error: no existe el archivo 'vehiculos.csv'.\n")
        return
...
def r_asistencia():
    ...
    # Asegurarse que el archivo exista.
    nombre_archivo = "datos/asistencias.csv"
    if not os.path.exists(nombre_archivo):
        asistencias = pd.DataFrame(columns=["dni", "fecha", "presente", "placa"])
        asistencias.to_csv(nombre_archivo, sep=";", index=False)
    ...
    try:
        ...
        print("Error: no existe el archivo 'estudiantes.csv'.\n")
        return
    ...
    # Validar que existan conductores registrados.
    try:
        ...
        print("Error: no existe el archivo 'conductores.csv'.\n")
        return
    ...
    while True:
        fecha = input("Ingrese la fecha de asistencia: ").strip()
    ...
    if presencia:
        ...
        # Todos los conductores de la ruta del estudiante.
        r_conduc = []
        for cond in l_conduc:
            if ruta_est in cond.rutas.split(","):
                r_conduc.append(cond)

        r_placas = []
        for cond in r_conduc:
        ...
        while True:
            try:
                ...
                if not 1 <= veh <= len(r_placas):
                    print(f"Error: ingrese una opción correcta. [1-{len(r_placas)}].\n")
                    continue
                break
            except ValueError:
                print("Error: ingreso inválido.\n")
